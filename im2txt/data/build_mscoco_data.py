...
def _to_sequence_example(image, decoder, vocab):
  ...
  # convert the caption into byte format in order to avoid conflict between python2 and 3
  for i in range(len(caption)):
      try:
          caption[i] = (caption[i].encode('utf-8'))
      except BaseException:
          continue
  caption_ids = [vocab.word_to_id(word.decode('utf-8')) for word in caption]
...
def _create_vocab(captions):
  ...
  with tf.gfile.FastGFile(FLAGS.word_counts_output_file, "wb") as f:
